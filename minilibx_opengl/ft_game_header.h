#ifndef FT_GAME_HEADER
# define FT_GAME_HEADER
# include <stdio.h>
# include <fcntl.h>
# include <stdlib.h>
# include <unistd.h>
...
typedef struct  s_floor_texture
{
    char        *path_floor_texture;
    void        *floor_texture;
    int         floor_texture_width;
    int         floor_texture_height;
    char        *addr_floor_texture;
    float       kef_floor_texture_width;
    float       kef_floor_texture_height;
    int         floor_texture_bits_per_pixel;
    int         floor_texture_line_length;
    int         floor_texture_endian; 
}               t_floor_texture;

typedef struct  s_ceilling_texture
{
    char        *path_ceilling_texture;
    void        *ceilling_texture;
    int         ceilling_texture_width;
    int         ceilling_texture_height;
    char        *addr_ceilling_texture;
    float       kef_ceilling_texture_width;
    float       kef_ceilling_texture_height;
    int         ceilling_texture_bits_per_pixel;
    int         ceilling_texture_line_length;
    int         ceilling_texture_endian; 
}               t_ceilling_texture;

typedef struct          s_list_npc
{
    int                 hp;
    int                 damage;
    int                 x;
...
int parsing_playing_field(t_map_list*map);


int create_map(t_win_list *win, t_map_list *map, t_list_main_hero_management *hero);
...
int create_hero(t_map_list *map, t_list_main_hero_management *hero, int i, int j);
void hero_position(t_map_list *map, t_list_main_hero_management *hero, int i, int j);
...
int validation_playing_field(t_map_list *map);
...
int start_game(t_win_list win_manager, t_map_list map, t_list_npc, t_list_main_hero_management hero);
...
int where_did_raycasting_fall(t_all_game_list *game, double x, double y, int wall);
int create_3d_map_line_sprite(t_all_game_list *game, int i);
int create_3d_map_texture(t_all_game_list *game, double h, double j, double x, double y, int a);
...
int create_3d_map_line(t_all_game_list *game, double h, double j, double x, double y, double a, int wall);

int print_n_t_line(t_all_game_list *game, double h, double x, float y, float line_texture);
int print_e_t_line(t_all_game_list *game, double h, double x, float y, float line_text);
int print_w_t_line(t_all_game_list *game, double h, double x, float y, float line_text);
int print_s_t_line(t_all_game_list *game, double h, double x, float y, float line_text);


//int visable_sprite(t_win_list *win, t_map_list *map, t_list_main_hero_management *hero, int x, int y, double h, double a);
int visable_sprite_ox(t_win_list *win, t_map_list *map, t_list_main_hero_management *hero, int x, int y, double h, double a);
int visable_sprite_oy(t_map_list *map, t_list_main_hero_management *hero, int x, int y, double h, double a);
int npc_map_parser(t_map_list *map);
int rendering_sprite(t_win_list *win, t_map_list *map, double h_offset, double v_offset, int sprite_size, int h);
